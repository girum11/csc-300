...
%         \item If discussion about √ípublic good√ì is used, there must be data to support it. Simply writing √íit benefits the general public because it would make many people happy√ì is insufficient.
%      \end{itemize}
%   \end{itemize}
...
Other tools attempting to expose this HTTP session hijacking problem came and went \cite{firesheep-day-later}. ``Very little has changed after each of these tools were released. They got their media hype, and then people forgot or didn’t care. For the most part, the tools were only used by tech-savy people, hackers and geeks'' \cite{firesheep-day-later}. 

Session hijacking had been brought up as an issue within the security community since 2004 \cite{firesheep-day-later}; software companies simply ignored the issue up until Firesheep's release \cite{firesheep-day-later}. Firesheep was another software tool aimed to \uline{demonstrate} (disclose) just how vulnerable websites were to HTTP session hijacking \cite{eric-butler}.
...
\subsubsection{The Project: Ensure Correct Usage of the HTTPS Protocol}
Butler described in his blog post accompanying Firesheep that the goal of Firesheep (his \uline{project}) was to ``demonstrate just how serious [HTTP session hijacking] is'' \cite{eric-butler}. In particular, Butler took issue with large websites using the HTTPS protocol incorrectly: ``sites fail to protect you because after you’ve authenticated, you're issued a cookie that identifies you throughout your browsing session, but if you think about it [hijacking that cookie is] just as good as your username/password for 99\% of the time'' \cite{firesheep-day-later}. 

Butler also took issue with another misuse of HTTPS: ``some sites support full encryption everywhere, but don't implement it properly by failing to set the `Secure' flag on authentication cookies, negating most of the benefits and leaving users at risk'' \cite{firesheep-day-later}.

Thus, Butler's primary motivation before releasing Firesheep was to assure that websites claiming to use HTTPS used it correctly to protect their users \cite{firesheep-day-later}.
...
For Butler to correct websites' usage of the HTTPS protocol, he had a few choices of methods \cite{hacking-ethics}.

\subsubsection*{Alternative 1: Wait for the software vendor to fix the flaw}
The first option Butler had in trying to correct usage of HTTPS is to tell people about the issue, and wait for websites to begin posting a fix \cite{hacking-ethics}. However, security experts had been warning websites of the dangers of HTTP session hijacking for six years before the release of Firesheep \cite{firesheep-day-later}.

\subsubsection*{Alternative 2: Expose the flaw}
The other option Butler had to correct site usage of HTTPS was to expose the flaw \cite{hacking-ethics}. The key in staying within White Hat boundaries here is ``determining what a reasonable time period is to wait for a fix from the software company'' \cite{hacking-ethics}. Butler decided that six years was long enough \cite{eric-butler}, and that the \uline{appropriate method was to expose the flaw} \cite{firesheep-day-later}.

\subsubsection{Substituted SE Code 3.05}
Ensure an appropriate method \uline{(software designed to expose the flaws of site HTTP vulnerabilities)} is used \uline{to correct the usage of the HTTPS protocol for user safety}.

\subsubsection{Conclusions drawn}
The \uline{appropriate method} to solve this problem was to release of software designed to expose HTTPS usage flaws \cite{eric-butler}, and thus accelerate the rate of response to HTTP session hijacking vulnerabilities \cite{eric-butler}.

As was stated in previous arguments in this report, HTTP session hijacking was a problem that was talked about to death among security professionals for years \cite{firesheep-day-later}. That is to say, companies had been outright ignoring this problem for (at least six \cite{firesheep-day-later}) years with no intention of changing their stance at the time of Firesheep's release. For a person in a position like Eric's it would be easy to argue that after six years of fruitless warnings, it was time to `raise the stakes' a little and force the hands of these non-compliant software companies \cite{firesheep-day-later}. If verbal signals for those six years turned out to not be an appropriate method for companies as (evidently) ignorant as this, then exposure of the gravity of the HTTP vulnerability was certainly the appropriate method to choose.

Following the guidelines of SE Code section 3.05, Butler thus had no choice but to use a more appropriate method to deal with this known issue \cite{se-code}: he had to raise the stakes a little and release Firesheep to the public \cite{hacking-ethics}. Software engineers under this code have an ethical responsibility to ensure that an appropriate method is used \cite{hacking-ethics}; SE Code 3.05 is yet another perspective on why \uline{Butler was ethical} and ethically obligated to release Firesheep \cite{se-code}.
